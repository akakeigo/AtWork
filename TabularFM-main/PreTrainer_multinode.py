"""
numeric_qwen_multinode.py
────────────────
Multi-node ready version of Qwen training with numeric tokens.
Fixed for distributed training across multiple nodes.
"""
import os
# Set environment variables BEFORE importing torch - CRITICAL for multi-node
os.environ["NCCL_ASYNC_ERROR_HANDLING"] = "1"
os.environ["CUDA_DEVICE_MAX_CONNECTIONS"] = "1"
os.environ["TRANSFORMERS_NO_META_DEVICE"] = "1"
os.environ["TOKENIZERS_PARALLELISM"] = "false"
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "expandable_segments:True"
os.environ["NCCL_TIMEOUT"] = "1800"  # 30 minutes for multi-node
os.environ["NCCL_BLOCKING_WAIT"] = "1"
os.environ["NCCL_DEBUG"] = "WARN"  # Change to INFO for debugging
...
def load_distributed_data(file_dir: str, rank: int, world_size: int):
    """
    Load data files distributed across ALL ranks (across all nodes).
    Fixed for multi-node training.
    """
    ...
    # CRITICAL: Only log from global rank 0
    if rank == 0:
        print(f"Found {len(all_files)} data files in {file_dir}")
        print(f"Distributing across {world_size} total processes")
    
    # Distribute files across ALL processes (not just within node)
    ...
    if rank == 0:
        print(f"[Rank {rank}] Loading files {start_idx} to {end_idx-1}: {[os.path.basename(f) for f in rank_files[:3]]}...")
    
    # Load tensors from assigned files
    all_tensors = []
    start_time = time.time()
    
    for file_path in tqdm(rank_files, desc=f"[Rank {rank}] Loading files", disable=(rank != 0)):
    ...
    print(f"[Rank {rank}] Loaded {len(all_tensors)} samples in {load_time:.1f}s")
    return all_tensors
...
class NumericDataProcessor:
    ...
    def make_dataloader(
        ...
        # When data is pre-sharded across ranks, we DON'T use DistributedSampler
        sampler = None
        ...
        # 5) DON'T prepare when using pre-sharded data!
        # Data is already distributed at file level
        if accelerator is not None and not precomputed_tensors:
...
# ---------------------------------------------------------------------
# 3)  Trainer - Enhanced for multi-node
# ---------------------------------------------------------------------
class QwenNumericTrainer:
    def __init__(
        ...
        grad_clip_norm: float = 1.0,  # Added for stability
    ):
        self.model       = model
        self.tok         = tokenizer
    ...
    def fit_epoch(self, dataloader: DataLoader, updates_per_epoch: int, epoch: int):
        """
        Runs one epoch with multi-node safe control flow and returns averaged losses.
        Enhanced with throughput tracking and node-level logging.
        """
        ...
        node_rank = global_rank // 8  # Assuming 8 GPUs per node
        ...
        pbar = tqdm(range(max_common_steps), desc=f"[Node {node_rank}] Training", disable=not is_main)
        for batch_idx in pbar:
            ...
            try:
                ...
                saw_oom_local = 1
                print(f"[Node {node_rank}, GPU {local_rank}] OOM on batch {batch_idx}, skipping...")
            ...
            if self.accelerator.sync_gradients:
                ...
                samples_processed += batch['input_ids'].shape[0] * self.grad_accum
                
                # Periodic synchronization for multi-node
                if steps % 100 == 0:
                ...
                if is_main and steps % log_every == 0:
                    ...
                    global_throughput = samples_processed * self.accelerator.num_processes / elapsed_time
                    ...
                    wandb.log({
                        ...
                        "throughput/samples_per_sec": global_throughput,
                        "throughput/tokens_per_sec": global_throughput * batch['input_ids'].shape[1],
                    })
                ...
                # Node-level logging every 100 steps
                if steps % 100 == 0 and local_rank == 0:
                    elapsed = time.time() - start_time
                    node_throughput = samples_processed / elapsed
                    print(f"[Node {node_rank}] Step {steps}, Loss: {tot_loss/steps:.4f}, "
                          f"Throughput: {node_throughput:.1f} samples/sec")
        ...
        epoch_save_dir = os.path.join("firstStagePretraining", f"epoch_{epoch}_final")
        if is_main:
            os.makedirs(epoch_save_dir, exist_ok=True)
        
        # All processes must call save_state
        self.accelerator.wait_for_everyone()
        self.accelerator.save_state(epoch_save_dir)
        self.accelerator.wait_for_everyone()
        
        if is_main:
            print(f"Saved epoch {epoch} final checkpoint to {epoch_save_dir}")
...
def main():
    ...
    print(f"[Node {node_rank}, Local GPU {local_rank}, Global rank {global_rank}] "
          f"Device: {accelerator.device}", flush=True)
    ...
    if is_main:
        ...
        wandb.init(project=args.wandb_project, name=args.wandb_run,
                   config={
                       "stage": "pretraining_tables",
                       "num_nodes": accelerator.num_processes // 8,
                       "total_gpus": accelerator.num_processes,
                       "effective_batch_size": args.batch_size * args.grad_accum_steps * accelerator.num_processes,
                   })
    ...
    # Scale learning rate by number of GPUs (optional but recommended for multi-node)
    if args.scale_lr:
        scaled_lr = args.lr * math.sqrt(accelerator.num_processes / 8)  # sqrt scaling
        if is_main:
            print(f"Scaled learning rate from {args.lr} to {scaled_lr} for {accelerator.num_processes} GPUs")
    else:
        scaled_lr = args.lr
    ...
    for ep in range(args.epochs):
        ...
        data_dir = f"./data/precomputed/epoch{int(ep+1)}/"
        ...
        dloader = processor.make_dataloader(
            ...
            use_distributed_sampler=False  # Data is already sharded
        )
        
        if scheduler is None:
            steps_per_epoch = len(dloader)
            print(f"[Rank {global_rank}] steps_per_epoch: {steps_per_epoch}")
            updates_per_epoch = math.ceil(steps_per_epoch / args.grad_accum_steps)
            print(f"[Rank {global_rank}] updates_per_epoch: {updates_per_epoch}")
